...
MD's [session defaults](session-defaults.toml) select Codex CLI, `gpt-5.6-sol`, and high reasoning effort. These are properties of a session, not MD's durable identity. Rig allows another runner or model adapter to occupy the seat later. The Emacs launcher starts Codex from this directory, grants the parent repository as an additional work area, and persists the process in tmux; see the [lifecycle decision](../knowledgebase/wiki/decisions/session-lifecycle.md).
